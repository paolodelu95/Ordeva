...
import { MarketplaceCanale, MarketplaceRigaDaAbbinare, MarketplaceStatistica } from '../../models';
...
@Component({
  ...
  template: `
    <div class="mkt-page">
      ...
      <!-- ── Statistiche vendite per canale ───────────────────────────────────── -->
      <div class="card">
          ...
          this.dialog.open(MarketplaceAbbinaDialogComponent, { data: res.daAbbinare, width: '640px', maxWidth: '96vw' })
            .afterClosed().subscribe((abbinamenti: (MarketplaceRigaDaAbbinare & { prodottoId: number })[] | undefined) => {
              ...
              this.ds.abbinaMarketplace(canaleUp, abbinamenti).subscribe({
                next: res2 => { this.loadStatistiche(); this.snack.open(this.i18n.tn('marketplace.msg.ordiniImportati', res.importati + res2.importati), '', { duration: 3000 }); },
