...
fn ebay_credenziali() -> Result<(String, String, String), ApiError> {
    let client_id = std::env::var("EBAY_CLIENT_ID").unwrap_or_default();
    let client_secret = std::env::var("EBAY_CLIENT_SECRET").unwrap_or_default();
    let runame = std::env::var("EBAY_RUNAME").unwrap_or_default();
